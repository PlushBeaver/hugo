	...
	if len(languages) == 1 {
		return hugofs.NewLingoFs(languageSet,
			append([]hugofs.MetaFs{
				hugofs.MetaFs{
					TheLang: language.Lang,
					TheFs:   afero.NewBasePathFs(source, absContentDir)}},
...
type themeFilesystems struct {
	...
	contentFilesystems []hugofs.MetaFs // Partitioned by language
...
func (b *sourceFilesystemsBuilder) createModFs(source afero.Fs, mod modules.Module) (afero.Fs, []hugofs.MetaFs, error) {

	var fromTo []hugofs.RootMapping
	var contentFilesystems []hugofs.MetaFs
	...
	for _, ft := range fromTo {
		if ft.Lang != "" {
			contentFilesystems = append(contentFilesystems, hugofs.MetaFs{
				TheLang: ft.Lang,
				TheFs:   newRealBase(afero.NewBasePathFs(rmfs, ft.To)),
			})
		}
	}

	return rmfs, contentFilesystems, nil
...
func (b *sourceFilesystemsBuilder) createThemeOverlayFs(source afero.Fs, mods modules.Modules) (afero.Fs, []hugofs.MetaFs, error) {
	if len(mods) == 0 {
